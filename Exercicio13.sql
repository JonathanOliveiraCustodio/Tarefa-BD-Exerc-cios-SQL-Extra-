...
c.marca,
c.modelo,
CAST(SUM(v.distanciaPercorrida) AS VARCHAR) + ' km' AS "Distancia Total Percorrida" 
FROM
carro c INNER JOIN viagem v
ON c.id = v.idCarro
GROUP BY c.marca, v.distanciaPercorrida,c.modelo 
ORDER BY v.distanciaPercorrida ASC

--Apresentar nome das empresas cuja soma total da distância percorrida pelos carros, em viagens, é superior a 50000 km
SELECT 
 e.nome,
 CASE WHEN SUM(v.distanciaPercorrida) > 50000 THEN SUM(v.distanciaPercorrida) END AS Distancia 
FROM 
 empresa e INNER JOIN carro c
 ON  e.id = c.idEmpresa 
  INNER JOIN viagem v
 ON v.idCarro = c.id 
  GROUP BY e.nome
...
SELECT
e.nome,
MAX(v.distanciaPercorrida) AS MaiorDistantancia
FROM
 empresa e INNER JOIN carro c
 ON e.id = c.idEmpresa
 INNER JOIN viagem v 
...
--Apresentar nome das empresas e a quantidade de carros cadastrados para cada empresa, desde que a empresa tenha 3 ou mais carros, a saída deve ser ordenada pela quantidade de carros, descrescente
 SELECT
  e.nome,
  CASE WHEN COUNT(c.id) > 3 THEN COUNT(c.id) END AS QtdCarros 
 FROM
 empresa e INNER JOIN carro c
 ON e.id = c.idEmpresa 
  GROUP BY e.nome 
...
SELECT 
 c.marca,
 c.modelo,
 COUNT(v.idCarro) AS QtdViagem
FROM 
carro c INNER JOIN viagem v
ON c.id = v.idCarro
 INNER JOIN empresa e
ON e.id = c.idEmpresa
 GROUP BY c.marca, c.modelo,v.idCarro,e.nome
  ORDER BY
  e.nome ASC,
  v.idCarro DESC;
...
SELECT
 ...
 --CASE WHEN v.distanciaPercorrida < 1000  THEN CAST(CHAR(08) (v.distanciaPercorrida * 10) AS DECIMAL(7,2)) ELSE V.distanciaPercorrida * 15 END AS ValorGanho
 CASE WHEN v.distanciaPercorrida < 1000 THEN CAST(v.distanciaPercorrida * 10 AS DECIMAL(7,2)) ELSE CAST(v.distanciaPercorrida * 15 AS DECIMAL(7,2)) END AS ValorGanho
FROM
carro c INNER JOIN viagem v
ON c.id = v.idCarro
 INNER JOIN empresa e
